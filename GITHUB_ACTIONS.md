...
수집·요약·커밋·푸시는 **자기 기계**에서 `scripts/publish.sh`가 한다. 스케줄러(macOS면
launchd, 리눅스면 cron)가 KST 00:00 / 08:00 / 16:00에 부른다. 이 저장소 클론에 `.env`
(`GROQ_API_KEY`)와 로그인된 `gh`가 있으면 된다. 로그는 `logs/publish.log`.
...
스케줄러 (정각)
  └─ scripts/publish.sh
       ...
       ├─ python build.py          ← 수집·요약·렌더
       ├─ 원격이 앞서 있으면 merge_remote_data.py 로 합친다
       ├─ git commit "뉴스 갱신 …" + push
       └─ 실패·열화·출처 침묵 → notify.sh 가 이슈를 연다
```

Actions에서 돌리지 않는 이유 둘:

- **러너 IP 차단.** 서브스택·digiato 같은 사이트가 깃허브 러너 대역을 막는다. 같은 주소를
  집에서 받으면 멀쩡하다. 9월 한 달 8%가 본문 없이 실렸다.
- **예약 지연.** 실측 60회 기준 슬롯별 중앙값이 110~301분 늦게 시작했다. 러너 부족이
  아니라 예약 이벤트 자체가 늦게 쏘는 것이라 파일 쪽에서 고칠 수 없다.

Actions에 남는 것:

- `watchdog.yml` — 세 시간마다 마지막 "뉴스 갱신" 커밋 나이를 보고 10시간을 넘으면
  이슈를 연다. 시크릿 없음. 수집 기계가 죽은 것을 알 길이 이것뿐이다.
- `daily.yml` — `workflow_dispatch` 전용. 수집 기계가 오래 멈췄을 때 손으로 한 번 돌리는
  비상용이다. 시크릿 `GROQ_API_KEY`가 있어야 돈다.
...
- 수집 기계의 `logs/publish.log`를 본다. "시작"만 있고 "끝"이 없으면 그 회차가 중간에 죽은 것이고, 시작 자체가 없으면 스케줄러가 안 부른 것이다.
- 빠진 회차를 메우려고 손으로 한 번 더 돌리지 않는다. 다음 회차가 그 사이 기사를 같이 가져온다. 추가로 돌리면 회차가 부풀고 Groq 하루 한도를 태운다.
- 기계가 오래 멈춘 상태면 Actions 탭에서 `daily.yml`을 손으로 한 번 돌린다(러너 IP 차단으로 본문 없는 기사가 섞인다).
...
- 실행 시각은 자기 기계의 스케줄러에 있다. `watchdog.yml`의 10시간 임계는 회차 간격(8시간)보다 커야 한다.
