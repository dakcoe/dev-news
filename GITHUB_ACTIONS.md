...
주 실행은 **바깥에서 `workflow_dispatch`로** 보낸다. 정각(KST 00:00 / 08:00 / 16:00)에
`gh workflow run daily.yml`을 부르는 스케줄러 하나면 된다. 보낸 지 몇 초 안에 시작한다.

GitHub의 `schedule`(cron)은 예비로만 남겨 둔다:

```yaml
schedule:
  - cron: "55 7,15,23 * * *"   # UTC. KST 16:55 / 00:55 / 08:55 — 주 실행 55분 뒤
```

왜 cron을 주로 쓰지 않나. GitHub 예약 실행은 실측 60회 기준 슬롯별 중앙값이 **110~301분**
늦게 시작했다 (KST 16시 회차가 가장 심했다). 러너가 부족해서가 아니다 — 예약 회차가 밀려
있는 동안 같은 파일의 수동 실행이 0초에 시작했다. 예약 이벤트 자체가 늦게 쏘는 것이라
파일 쪽에서 고칠 방법이 없다.

cron이 도착하면 `guard` 작업이 "최근 8시간 안에 성공했거나 진행 중인 `workflow_dispatch`
회차가 있었나"를 보고, 있으면 즉시 끝낸다. 없을 때(바깥 스케줄러가 죽었거나 그 회차가
실패했을 때)만 실제로 수집한다.

cron을 주 실행 **뒤**(정각 + 55분)에 두는 것이 중요하다. 앞에 두면 예약이 제때 온 날에
guard 창에 아직 아무것도 없어 둘 다 돈다. 8시간인 이유: 예약이 가장 늦었을 때가 398분이라
정각의 수동 실행은 도착 시점에서 최대 7시간 33분 전이고, 직전 슬롯은 그보다 8시간 더
앞이라 잡히지 않는다.

:55인 이유는 정각 쏠림을 피하려는 것이다. 전 세계가 정각에 큐를 채워 정각일수록 오래
밀린다. `workflow_dispatch`는 큐를 타지 않으므로 정각에 보내도 된다.

- 저장소에 60일간 커밋이 없으면 GitHub가 스케줄을 자동으로 끈다. 이 프로젝트는 매 회차 커밋하므로 해당 없다.
- 두 경로의 지연을 비교하고 싶으면 Actions 탭에서 `schedule` 회차와 `workflow_dispatch` 회차의 시작 시각을 나란히 보면 된다. 예약 회차는 guard에서 끝나므로 20초 안에 끝난다.
...
- 바깥 스케줄러 쪽 로그에 "보냄"이 있는데 Actions에 실행 기록이 없으면 GitHub 쪽, 로그 자체가 없으면 스케줄러 쪽이다.
- 빠진 회차를 메우려고 손으로 한 번 더 돌리지 않는다. 다음 회차가 그 사이 기사를 같이 가져온다. 추가로 돌리면 회차가 부풀고 Groq 하루 한도를 태운다.
- cron 예비분이 대신 돌았다면 몇 시간 늦게라도 게시된다. 그것도 없으면 Actions 탭에서 워크플로가 disabled인지 본다. fork한 저장소는 스케줄이 기본으로 꺼져 있다.
...
- cron을 바꾸면 UTC 기준임을 잊지 않는다. 주 실행 시각은 바깥 스케줄러에 있으니 거기도 같이 바꾼다.
- guard의 8시간 창은 "정각 + 55분 + 최대 지연(398분)"보다 크고 "회차 간격 + 55분"보다 작아야 한다. 회차를 더 자주 돌리게 바꾸면 cron 위치와 이 값을 같이 손본다.
