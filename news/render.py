...
def about_copy(enabled: dict | None) -> list[dict]:
    ...
    return [
        ...
        {"id": "privacy", "title": "개인정보 처리",
         "html": "<p>로그인하지 않으면 보관함과 읽음 표시는 브라우저의 localStorage에만 저장되며 "
                 "외부로 전송되지 않습니다.</p>"
                 "<p>GitHub 로그인을 선택한 경우에 한해, 여러 기기에서 같은 목록을 보기 위해 다음을 "
                 "서버에 보관합니다 — GitHub 계정의 숫자 ID, 보관한 기사의 주소와 제목, 읽음으로 표시한 "
                 "기사의 주소. 이름·이메일·프로필 사진은 저장하지 않으며, GitHub 액세스 토큰도 저장하지 "
                 "않습니다. 읽음 표시는 계정당 최근 1000건까지만 남습니다. 소개 화면에서 로그아웃할 수 "
                 "있습니다.</p>"
