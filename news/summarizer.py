...
FOREIGN_RE = re.compile(
    ...
    # 악센트 붙은 로마자 (ü é ñ ł …). 사람 이름·지명에 흔하다. 빠져 있어서
    # "Jürgen" 의 ü 가 외국 문자로 걸려 치환 단계가 "J유rgen" 으로 바꿨다.
    # × (U+00D7)·÷ (U+00F7) 는 글자가 아니라서 범위에서 뺀다.
    r"À-ÖØ-öø-ɏ"
    r"£¥°±·×"      # £ ¥ ° ± · ×
    r"‐-―‘’“”…"  # 하이픈·대시류, 따옴표, 말줄임
...
# 영어 원제를 복사한 채 게재하지 않도록 제목에도 한국어를 요구한다 (fix-untranslated-titles).
...
# 요약을 반복하는 한 줄로 끝나지 않도록 기사 근거를 따로 풀어 쓴다 (thicken-why).
...
# (mvanhorn / last30days-skill 건: 제목 설명과 요약이 서로 다른 물건을 가리켰다).
_OWNER_REPO_RE = re.compile(r"^[\w.+-]+\s*/\s*[\w.+-]+$")
_IDENT_TOKEN_RE = re.compile(r"^[\w.+-]*[\d_.+-][\w.+-]*$")
_MAX_IDENT_TOKENS = 3


def looks_like_identifier_title(title: str) -> bool:
    """제목 전체가 저장소명·패키지명 같은 고유명사인가.

    owner / repo 형태, 또는 숫자·구분자가 섞인 토큰이 세 개 이하로만 있는 제목.
    평범한 단어가 하나라도 섞이면 문장형으로 본다 — 그때는 옮기기만 해야 한다.
    """
    t = (title or "").strip()
    if not t:
        return False
    if _OWNER_REPO_RE.match(t):
        return True
    tokens = t.split()
    if not tokens or len(tokens) > _MAX_IDENT_TOKENS:
        return False
