...
def test_설명문에만_있는_차단어로는_막지_않는다():
    """차단은 제목만 본다.

    예전에는 설명문까지 봐서, 낱말 하나가 스친 것으로 개발 기사가 빠졌다.
    게재분 2,060건으로 재보니 이 변경으로 4건이 새로 통과하는데 셋은 감시
    카메라·키워드 기반 연구비 취소·플랫폼 수익화라 이 사이트가 다룰 갈래고,
    하나(갈매기 기사)만 무관하다. 3건을 살리고 1건을 들이는 맞바꿈이다.

    설명문은 통과 판정에는 계속 쓴다 — 제목이 짧은 기사는 그게 유일한 근거다.
    """
    assert _kept("기록적 수준으로 빠르게 성장하는 2026년 슈퍼 엘니뇨",
                 description="전 세계 기후에 강력한 영향을 미치는 대형 현상")


def test_제목의_차단어는_그대로_막는다():
    assert not _kept("2026년 선거 후보 토론 일정 확정")
    assert not _kept("올림픽 중계 방송권 사상 최고가 낙찰")
